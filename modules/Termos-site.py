...
Cria um servidor web independente para os termos
"""
...
class TermosSite:
    ...
    async def start(self):
        """Inicia o servidor web dos termos"""
        try:
            self.app = web.Application()
            
            async def handle_termos(request):
                """Página principal de termos"""
                # Link da imagem limpo (sem os parâmetros ?ex=...)
                imagem_url = "https://cdn.discordapp.com/attachments/1477698899237011598/1487026151254790275/Design_sem_nome_3.png"
                ...
                <body>
                    ...
                    <div class="footer">
                        <p>© 2025 WaveX - Todos os direitos reservados</p>
            ...
            # Registrar a rota
            self.app.router.add_get('/', handle_termos)
            self.app.router.add_get('/termos', handle_termos)
            
            # Iniciar o servidor
            self.runner = web.AppRunner(self.app)
            await self.runner.setup()
            
            # Usar porta 10002 para o site de termos (diferente do keep-alive)
            port = int(os.environ.get('TERMOS_PORT', 10002))
            self.site = web.TCPSite(self.runner, '0.0.0.0', port)
            await self.site.start()
            
            print(f"📄 Site de termos rodando na porta {port}")
            print(f"📌 Acesse: https://seu-dominio.com:{port}/termos")
            
        except Exception as e:
            print(f"⚠️ Erro ao iniciar site de termos: {e}")
    
    async def stop(self):
        """Para o servidor web dos termos"""
        if self.site:
            await self.site.stop()
        if self.runner:
            await self.runner.cleanup()

# Variável global para a instância
termos_site = None
